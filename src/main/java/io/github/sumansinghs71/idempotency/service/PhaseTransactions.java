...
import io.github.sumansinghs71.idempotency.repository.AuditLogRepository;
import io.github.sumansinghs71.idempotency.repository.IdempotencyKeyRepository;
import io.github.sumansinghs71.idempotency.repository.RideRepository;
...
public class PhaseTransactions {
    ...
    /**
     * Deletes an expired row for {@code (userId, key)} so a replacement can be
     * inserted. Called only from the concurrent-insert retry path — if the
     * 23505 was a genuine race rather than an expired leftover, the row is not
     * expired and this is a no-op.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public int purgeExpiredKey(long userId, String key) {
        return keyRepo.deleteExpiredForKey(userId, key, Instant.now());
    }
